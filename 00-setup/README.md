...
| `WARN Shell: Did not find winutils.exe ... HADOOP_HOME and hadoop.home.dir are unset` | Spark bundles Hadoop's client libraries, which expect Hadoop's native Windows shims (`winutils.exe`, `hadoop.dll`) for filesystem operations, even for purely local runs. | Download `winutils.exe` + `hadoop.dll` matching your bundled Hadoop version (check `pyspark/jars/hadoop-client-api-*.jar` for the version) from a trusted community mirror (search `cdarlint/winutils` on GitHub — a well-known, widely-used source for these), place them in `<somewhere>\hadoop\bin\`, and set `HADOOP_HOME` to `<somewhere>\hadoop`. |
